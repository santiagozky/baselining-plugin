...
@Mojo(name = "baseline30", defaultPhase = LifecyclePhase.VERIFY)
@Execute(phase = LifecyclePhase.VERIFY, goal = "baseline")
public class Baseline30Mojo extends AbstractMojo {
	...
	private File getLastArtifact() throws MojoExecutionException {
        org.sonatype.aether.version.Version v = getLastVersion();
		if (v == null) {
			return null;
		}
	 ...
	 * @throws MojoExecutionException
	 */
    private org.sonatype.aether.version.Version getLastVersion()
			throws MojoExecutionException {
		...
		try {
			rangeResult = repoSystem.resolveVersionRange(repoSession,
					rangeRequest);
            List<org.sonatype.aether.version.Version> versions = rangeResult
					.getVersions();
			getLog().debug(
            ...
            org.sonatype.aether.version.Version lastVersion = versions
					.get(versions.size() - 1);
